...
def main(args): 
    ...
    if args.mode == 'properties':
        output_path = f'{args.output_dir}/property/property_{args.start}_{args.end}_{dt_string}.jsonl'
    elif args.mode == 'pbt':
        output_path = f'{args.output_dir}/pbt/pbt_{args.start}_{args.end}_{dt_string}.jsonl'
    else:
    ...
    for i in progress_bar:
        ...
        # === generate properties ===
        if args.mode == 'properties':
            question = PROPERTIES_PROMPT_DOC.format(function_name=eval_data[i]['function_name'], api_documentation=eval_data[i]['api_doc'])
            messages = [
                {"role": "system", "content": system_prompt},
                {"role": "user", "content": question} 
            ]
        elif args.mode == 'pbt':
            question_prev = PROPERTIES_PROMPT_DOC.format(function_name=eval_data[i]['function_name'], api_documentation=eval_data[i]['api_doc'])
            assistant_props = eval_data[i]['properties'][0]
            question = PBT_PROPERTIES_PROMPT_DOC.format(function_name=eval_data[i]['function_name'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_path', type=str, default='output_jsonl/property/property_0_30_10-30-20-29.jsonl')
    parser.add_argument('--output_dir', type=str, default='output_jsonl')
